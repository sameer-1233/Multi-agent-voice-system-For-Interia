...
**Develop an AI-powered voice assistant for INTERIA, an interior design company. The assistant should provide recommendations, answer queries, and interact conversationally with users interested in interior design. The system should leverage AI/ML technologies and integrate with INTERIA’s knowledge base.**
...
Answering questions about Interia’s design services

Understanding customer intent through AI

Transcribing voice to text using Whisper ASR

Generating human-like responses using OpenAI’s LLM
...
dialog_flow = {
    "greeting": "Hello! I’m Interia’s virtual design assistant. Are you exploring interior design ideas or ready for a consultation?",
    ...
    "closing": "Awesome. We’ll contact you soon. Have a great day!"
}
...
os.environ['OPENAI_API_KEY'] = 'Your openai_api key'
...
dialog_flow = {
    "greeting": "Hello! I’m sammy from Interia. I'm reaching out regarding your interior design needs.",
...
def search_projects(user_query, k=3):
    ...
    print("Top matches:\n")
    for i in indices[0]:
        print(f"📍 {project_descriptions[i]}")
...
project_data = [
    ...
    "For a 3 BHK home, the interior design cost typically ranges between ₹8 to ₹15 lakhs depending on style and materials used. Premium designs may cost more. Would you like a personalized estimate?",
...
print("🧠 Matched Project:", response)
...
prompt_template = PromptTemplate(
    ...
    template=(
        ...
        "gathering information on project scope, budget (minimum ₹30 lakhs), timeline, and design needs.\n\n"
...
response = create_new_lead("Ayushi kaur", "Chandigarh", "20 Lakhs", "Modular Kitchen")
...
full_voice_ai_pipeline("Can you give me a cost estimate for a 3 bhk design.wav")

full_voice_ai_pipeline("How can decorate my living room in a minimalistic.wav")

"""Correction* It is minimalistic style not mini ballistic style .

### 🛡️ Error Handling for Airtable Lead Updates
...
"""### ✅ Sample Airtable Entry (Created from n8n Workflow)
...
| Item | Status |
|------|--------|
| Full voice flow (ASR → LLM → TTS) | ✔️ Confirmed |
| Airtable integration (lead create + status update) | ✔️ Confirmed |
| n8n low-code automation workflow built | ✔️ Confirmed |
| Code is clean and commented | ✔️ Confirmed |
| Markdown + explanations included | ✔️ Confirmed |
| All screenshots uploaded | ✔️ Confirmed |
| API keys hidden or marked | ✔️ Done |

### 🔄 System Workflow Overview

```mermaid
graph TD
    A[🎤 Voice Input] --> B[🧠 ASR (Speech-to-Text) - Whisper]
    B --> C[🔍 FAISS Search (Find Matching Project)]
    C --> D[💬 LLM (OpenAI GPT) - Response Generation]
    D --> E[🔊 TTS (Text-to-Speech) - Voice Response]
    D --> F[📋 Airtable - Lead Creation]
    F --> G[⚙️ n8n Workflow (Lead Update/Sync)]
